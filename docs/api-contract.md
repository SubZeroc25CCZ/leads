...
| `orlando_code_enforcement` | Orange | code_violation | Socrata של Orlando דרך ה-adapter שלנו (‏~2,100 תיקים/30 יום) |
| `deerfield_code_cases` | Broward | code_violation | ArcGIS של Deerfield Beach; field_map מלא כולל owner (‏~330 תיקים/30 יום) |

מחוזות שנחקרו ונפסלו (12.08.2026): Jacksonville/Duval (GIS נעול ב-token, אין feed
ציבורי), Hillsborough/Tampa (השכבה הציבורית קפואה — אין שדה תאריך, עדכון אחרון
06/2026; התיקים החיים ב-Accela בלי API), Fort Lauderdale (מערכת הת tracker הוצאה
משימוש ב-2019). Deerfield Beach הוא הכניסה שלנו ל-Broward בינתיים.
