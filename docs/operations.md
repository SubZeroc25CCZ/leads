...
**⚠️ תלות פריסה:** התיקון בגיט אבל ה-probe הפרוס עדיין מחזיר את הכתובת הגולמית.
עד redeploy של `leadmachine-probe` — **שלב ה-upsert בלולאה 2 חייב להחיל את אותו
נרמול** (`s.replace(/(\bFL\s+\d{5})\b.*$/i, '$1')`) לפני חישוב `address_key`,
אחרת הקציר הבא יכניס את אותם 28 נכסים שוב כשורות חדשות.
